...
exports.addInterview = async (req, res, next) => {
  try {
    ...
    if (
      ...
      company.maximum_interview <= company.interviews.length
    ) {
      return res.status(400).json({
        success: false,
        message: `The company with ID ${req.params.companyId} has already made ${company.maximum_interview} interviews that reached maximum interview.`,
      });
    }
